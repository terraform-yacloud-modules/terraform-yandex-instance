...
| <a name="input_metadata_options"></a> [metadata\_options](#input\_metadata\_options) | Options to configure access to instance's metadata (0 = DISABLED, 1 = ENABLED for endpoints; 0 = OPTIONAL, 1 = REQUIRED for tokens) | <pre>object({<br/>    gce_http_endpoint    = optional(number)<br/>    aws_v1_http_endpoint = optional(number)<br/>    gce_http_token       = optional(number)<br/>    aws_v1_http_token    = optional(number)<br/>  })</pre> | `null` | no |
...
| <a name="input_secondary_disks"></a> [secondary\_disks](#input\_secondary\_disks) | Additional disks with params | <pre>map(object({<br/>    enabled     = optional(bool, true)<br/>    auto_delete = optional(bool, false)<br/>    mode        = optional(string)<br/>    labels      = optional(map(string), {})<br/>    type        = optional(string, "network-hdd")<br/>    size        = optional(number, 10)<br/>    block_size  = optional(number, 4096)<br/>    device_name = optional(string)<br/>    snapshot_id = optional(string)<br/>  }))</pre> | `{}` | no |
